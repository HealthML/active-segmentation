...
from functional import BCELoss, DiceLoss, FalsePositiveLoss
...
class TestDiceLoss(unittest.TestCase):
    ...
    def test_standard_case(self):
        ...
        self._test_dice_loss(
            standard_slice_1, standard_slice_2, smoothing=0, reduction="mean"
        )
        self._test_dice_loss(
            standard_slice_1, standard_slice_2, smoothing=0, reduction="sum"
        )

        self._test_dice_loss(standard_slice_1, standard_slice_2, smoothing=1)
        self._test_dice_loss(
            standard_slice_1, standard_slice_2, smoothing=1, reduction="mean"
        )
        self._test_dice_loss(
            standard_slice_1, standard_slice_2, smoothing=1, reduction="sum"
        )
    ...
    def test_all_false(self):
        ...
        self._test_dice_loss(
            ...
            smoothing=1,
            reduction="mean",
            expected_loss=torch.tensor(-1.0 / (1.0 + fp + fn)),
        )
        self._test_dice_loss(
            slice_all_false,
            slice_all_false,
            smoothing=1,
            reduction="sum",
            expected_loss=torch.tensor(-2.0 / (1.0 + fp + fn)),
        )

    def test_no_true_positives(self):
        self._test_dice_loss(
            slice_no_true_positives,
            slice_no_true_positives,
            smoothing=0,
            expected_loss=torch.tensor([[0.0], [0.0]]),
        )
    ...
    def test_no_true_negatives(self):
        ...
        self._test_dice_loss(
            ...
            smoothing=0,
            reduction="mean",
            expected_loss=torch.tensor(-2.0 * tp / (2.0 * tp + fp + fn)),
        )
        self._test_dice_loss(
            slice_no_true_negatives,
            slice_no_true_negatives,
            smoothing=0,
            reduction="sum",
            expected_loss=torch.tensor(-4.0 * tp / (2.0 * tp + fp + fn)),
        )
    ...
    def test_multi_class_prediction(self):
        ...
        torch.testing.assert_allclose(
            loss,
            torch.tensor(((loss_1 + loss_2) / 2 + loss_1) / 2),
