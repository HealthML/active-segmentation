...
class TestDiceScore(unittest.TestCase):
    ...
    def _expected_dice_score(
        ...
        Args:
            ...
            reduction (string): A method to reduce metric Dice scores of multiple classes.
        ...
        Returns:
            torch.Tensor: List of expected Dice scores.
        """
                    ...
                    for epsilon in [0, 1]:
                        ...
                        if epsilon == 0:
                            if convert_to_one_hot is False or reduction != "none":
                                ...
                                self.assertTrue(
                                    torch.all(torch.isnan(score_from_function)[1:]),
                                    f"Functional implementation correctly computes Dice score for {task_type} tasks "
                                    f"when there are only TN and {test_case_description}.",
                                )
                        else:
                        ...
                        if epsilon == 0:
                            if convert_to_one_hot is False or reduction != "none":
                                ...
                                self.assertTrue(
                                    torch.all(torch.isnan(score_from_module)[1:]),
